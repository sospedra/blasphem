...
```bash
cargo run --release -p toxtrain -- setup
```
...
```bash
cargo run --release -p toxtrain -- setup --languages all
```
...
```bash
cargo run --release -p toxcheck --bin toxcheck -- check --language EN --text "You are an idiot"
cargo run --release -p toxcheck --bin toxcheck -- check --language EN --text "I will kill you"
cargo run --release -p toxcheck --bin toxcheck -- check --language EN --text '"You are an idiot," she said'
cargo run --release -p toxcheck --bin toxcheck -- check --language ES --text "Te voy a matar"
cargo run --release -p toxcheck --bin toxcheck -- check --language ES --text "ojala se muera toda tu familia"
cargo run --release -p toxcheck --bin toxcheck -- check --language ES --text "No te voy a matar"
```
...
```bash
cargo run --release -p toxcheck --bin toxcheck -- check --language EN --reply-target person --text "Idiot"
```
...
```bash
cargo run --release --bin toxcheck -- check --language AUTO --text "Cuando te vea, te rompo los dientes"
```
...
Use an explicit code to bypass ELDC.
...
```bash
cargo run --release -p toxtrain -- eval \
...
```bash
cargo run --release --locked -p toxtrain -- cli-smoke \
  --model-manifest resources/models/multilingual-v2/manifest.json \
  --hurtlex-root data/raw-v1/hurtlex \
...
The `toxtrain setup`, `observe`, and `acquire` commands use the network.
...
The default build includes ELDC and all 15 toxicity packs.
...
Build the explicit-only module without ELDC:
...
The shared ELDC table does not shrink in proportion to the selected pack count.

See `docs/eldc-auto-report.md` for route, latency, size, and browser evidence.
