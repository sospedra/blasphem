...
**Go, Python, and Java, 2026-09-04.** The same contract in three more languages, all over the same core. `packages/go` is cgo over `crates/blasphem-ffi`: `Init`, `Judge`, `Ready`, `Close`, `New` returning an `*Instance`, `Options{Locales, Assets, Packs fs.FS, DisableDetection, Grawlix}`, `*Error{Code, Message}`. `packages/python` is a PyO3 extension (`crates/blasphem-python`, a standalone Cargo workspace so `cargo test --workspace` never links against an interpreter) under a pure-Python layer: `init`, `judge`, `ready`, `close`, `Judge`, `Judgement` dataclass, `BlasphemError.code`; `packages/python-packs` is the data wheel `blasphem` falls back to. `packages/java` uses the Foreign Function and Memory API, Java 22 or later, no JNI: `Blasphem.init/judge/ready/close`, `Judge.create`, `Options` record with withers, `Judgement` record with `Optional`s, `BlasphemException.code()`. Every binding reads the manifest itself and hands digests to Rust; the locale tables are generated by `blasphem-train locales-table --format go|python|java` and pinned by a test. The C ABI gained `blasphem_builder_error`, and a failed `blasphem_builder_build` now leaves the builder alive, because Go goroutines and Java virtual threads migrate between OS threads and a thread-local last-error slot would lie to them. Verified on 2026-09-04: each example judged the README text to `score 0.64` with `locale en` and the grawlix, failed open on Korean, and reported `BLASPHEM_LOCALE_UNSUPPORTED` for `xx`; Go through `go run`, Python through `maturin develop` into a Python 3.14 venv plus the data wheel without `assets`, Java compiled with `javac --release 22 -Werror` on JDK 25.

**Svelte and Solid, 2026-09-04.** Verified in throwaway `create-vite` apps (`svelte-ts`, `solid-ts`) installed from `pnpm pack` tarballs: both built, and in Chromium 151 and WebKit 26.5 each judged the README text to `score 0.64`, fetching only `manifest.json`, `blasphem_bg.wasm`, and the `en` and `es` files. pnpm skipped the unpublished `@blasphem/node-*` optional dependencies without error. SvelteKit and SolidStart server routes are documented, not run.

**Go over WebAssembly, 2026-09-04.** `packages/go` dropped cgo. Go has no registry: `go get` copies the repository at a tag and cannot fetch `libblasphem_ffi.a`, so a cgo module needs one static archive per platform committed to git, 8.19 MB each, five platforms, every release, forever. Instead `crates/blasphem-ffi` compiles unchanged for `wasm32-unknown-unknown` (1.26 MB, no packs) and ships as `packages/go/blasphem_ffi.wasm`, embedded with `//go:embed` and run by wazero 1.12.0; the C ABI gained `blasphem_alloc` and `blasphem_free` so a host without shared memory can stage pointer arguments. `CGO_ENABLED=0`, cross-compiling, and Windows work. Every `Instance` owns one module instance behind one mutex; Rubén chose the mutex over an instance pool. Measured on darwin/arm64 with 600 corpus messages, en plus es, detection and grawlix on: cgo 57 µs per judge and 41 ms init, wasm 107 µs and 225 ms init, of which 152 ms is the one-time compile; verdicts identical on all 600. Rubén ruled 50 µs immaterial for a pre-send nudge. CI rebuilds the wasm on Linux and macOS and fails when the bytes differ from the committed file, which also tests whether the pinned toolchain is byte-reproducible across hosts. The Go module now needs Go 1.25, wazero's floor. Separate repositories for platform binaries, the DuckDB and OpenDAL pattern, were rejected: no new repos.


**One version everywhere, 2026-09-04.** Rubén reversed the independent-release decision for the packs: `blasphem` and `@blasphem/packs` now carry the same version, because two numbers for one product confuse users. The format version inside each pack stays as the compatibility check, but releases move in lockstep. The single source is `[workspace.package].version` in the root `Cargo.toml`; every crate inherits it with `version.workspace = true`, and `blasphem-train sync-versions` writes it into the 24 manifests that cannot inherit: the npm packages and their platform manifests under `packages/node/npm` and `packages/cli/npm`, both `pyproject.toml` files, `pom.xml`, and the standalone `crates/blasphem-python/Cargo.toml`. `sync-versions --check` and a unit test fail when any mirror drifts. The JavaScript side collapsed `VERSIONS { blasphem, packs }` to one exported `VERSION`, and `scripts/build.mjs` refuses to build when the two `package.json` versions differ. Go needs nothing; its version is the git tag.
