use toxcheck::{
    AnalysisContext, CandidateViewKind, Detector, LexiconEntry, MatchLevel, PolicyAction,
    ReplyTarget, RuleId, parse_hurtlex,
};
...
fn scores_duplicate_conservative_rows_as_one_lexical_event() {
    ...
    let result = detector.analyze("You idiot.", AnalysisContext::for_language("EN"));
...
fn selects_the_strongest_level_for_duplicate_rows() {
    ...
    let result = detector.analyze("Idiot.", AnalysisContext::for_language("EN"));
...
fn scores_each_candidate_before_selecting_the_strongest_match() {
    ...
    let result = detector.analyze("іdiot", AnalysisContext::for_language("EN"));
...
fn identity_evidence_uses_the_identity_candidate_when_a_stronger_nonidentity_view_shares_its_span()
{
    ...
    let result = detector.analyze("Immigrants are іdiot.", AnalysisContext::for_language("EN"));
...
fn lexical_evidence_copies_evasion_candidate_provenance() {
    ...
    let result = detector.analyze("i.d.i.o.t", AnalysisContext::for_language("EN"));
    let evidence = result
        .evidence
        .iter()
        .find(|item| item.rule_id == RuleId::LexicalMatch)
...
fn lexical_evidence_keeps_normalized_and_raw_offsets_distinct() {
    ...
    let result = detector.analyze("ERES ESTÚPIDO", AnalysisContext::for_language("ES"));
    let evidence = result
        .evidence
        .iter()
        .find(|item| item.rule_id == RuleId::LexicalMatch)
...
fn excluded_evidence_copies_evasion_candidate_provenance() {
    ...
    let result = detector.analyze("l o v e", AnalysisContext::for_language("EN"));
    let evidence = result
        .evidence
        .iter()
...
fn rule_only_evidence_has_no_candidate_provenance() {
    let result = english_detector().analyze("I will kill you", AnalysisContext::for_language("EN"));
    let evidence = result
        .evidence
        .iter()
...
fn caps_duplicate_suppressed_rows_as_one_lexical_event() {
    ...
    let result = detector.analyze("You are not an idiot.", AnalysisContext::for_language("EN"));
...
fn spanish_hostile_wishes_score_threat_language() {
    ...
    for text in [
        ...
        let result = detector.analyze(text, AnalysisContext::for_language("ES"));
...
fn spanish_semantic_threat_variants_do_not_accumulate() {
    let result = spanish_detector().analyze(
...
fn spanish_future_assault_and_desired_death_score_threat_language() {
    ...
    let assault = detector.analyze(
        "Te partiré los dientes.",
        AnalysisContext::for_language("ES"),
    );
    let desired_death = detector.analyze(
        "Tu familia debería morir.",
        AnalysisContext::for_language("ES"),
    );
...
fn spanish_negation_suppresses_the_complete_intent_frame() {
    let result =
        spanish_detector().analyze("No te voy a matar.", AnalysisContext::for_language("ES"));
...
fn scores_a_direct_insult_as_targeted_abuse() {
    let result =
        english_detector().analyze("You are an idiot.", AnalysisContext::for_language("EN"));
...
fn nudge_result_uses_the_maximum_rule_score_and_fixed_threshold() {
    let result =
        english_detector().analyze("I will kill you.", AnalysisContext::for_language("EN"));
...
fn nudge_boolean_is_derived_from_the_score_threshold() {
    let result = english_detector().analyze("Idiot.", AnalysisContext::for_language("EN"));
...
#[test]
fn explicit_spanish_analysis_loads_the_compiled_sparse_channel() {
    let result = spanish_detector().analyze(
        "La gente del pueblo habló con la policía.",
        AnalysisContext::for_language("ES"),
    );

    assert_eq!(result.sparse_score, Some(15));
    assert_eq!(result.nudge().score, 15);
    assert!(!result.nudge().should_nudge);
}

#[test]
fn non_spanish_analysis_does_not_load_the_spanish_sparse_channel() {
    let result = english_detector().analyze("Thank you.", AnalysisContext::for_language("EN"));
...
fn uses_the_reply_target_for_a_bare_insult() {
    let result = english_detector().analyze(
        "Idiot.",
        AnalysisContext {
            language: Some("EN"),
            reply_target: ReplyTarget::Person,
...
fn detects_a_threat_without_an_intent_marker() {
    let result = english_detector().analyze("I kill you.", AnalysisContext::for_language("EN"));
...
fn detects_kill_yourself_without_an_intent_marker() {
    let result = english_detector().analyze("Kill yourself.", AnalysisContext::for_language("EN"));
...
fn an_unrelated_negator_before_a_threat_does_not_suppress_the_threat() {
    let result =
        english_detector().analyze("No, I will kill you.", AnalysisContext::for_language("EN"));
...
fn an_unrelated_report_before_a_threat_does_not_suppress_the_threat() {
    for text in [
        ...
        let result = english_detector().analyze(text, AnalysisContext::for_language("EN"));
...
fn compact_rules_scope_reports_and_quotes_to_the_matched_clause() {
    ...
    for (language, text) in [
        ...
        let result = detector.analyze(text, AnalysisContext::for_language(language));
...
fn suppresses_a_negated_insult_but_keeps_the_lexical_match() {
    let result =
        english_detector().analyze("You are not an idiot.", AnalysisContext::for_language("EN"));
...
fn suppresses_an_insult_after_a_multi_token_normalized_negator() {
    let result =
        english_detector().analyze("You don't idiot.", AnalysisContext::for_language("EN"));
...
fn a_neutral_group_list_does_not_form_an_identity_attack() {
    let result = local_hurtlex_detector().analyze(
...
fn every_language_pack_distinguishes_a_list_from_a_direct_identity_relation() {
    ...
    for (language, lemma, neutral_text, hostile_text) in fixtures {
        ...
        let neutral = detector.analyze(neutral_text, AnalysisContext::for_language(language));
        let hostile = detector.analyze(hostile_text, AnalysisContext::for_language(language));
...
fn positive_sentiment_does_not_cancel_another_clauses_negative_sentiment() {
    let result = english_detector().analyze("Awful. Good.", AnalysisContext::for_language("EN"));
...
fn applies_a_multi_token_normalized_diminisher_to_sentiment() {
    let result = english_detector().analyze(
        ...
        AnalysisContext::for_language("FR"),
    );
...
fn does_not_target_an_insult_across_clauses() {
    let result =
        english_detector().analyze("You are kind. Idiot.", AnalysisContext::for_language("EN"));
...
fn reactivates_the_audited_german_direct_abuse_phrase() {
    ...
    let result = detector.analyze("Du Hund.", AnalysisContext::for_language("DE"));
...
fn keeps_neutral_german_hund_contexts_excluded() {
    ...
    for text in [
        ...
        let result = detector.analyze(text, AnalysisContext::for_language("DE"));
...
fn excludes_italian_gente_in_explicit_and_auto_inclusive_modes() {
    ...
    for context in [
        AnalysisContext::for_language("IT"),
        AnalysisContext {
            language: None,
            reply_target: ReplyTarget::Unknown,
        },
    ] {
        let result = detector.analyze("La gente aspetta.", context);
...
#[test]
fn local_hurtlex_keeps_explicit_profanity_active_in_all_language_packs() {
    let fixtures = [
        ("EN", "fuck"),
        ("DE", "arschloch"),
        ("ES", "puta"),
    ...
    for (language, text) in fixtures {
        let result =
            local_hurtlex_detector().analyze(text, AnalysisContext::for_language(language));
...
fn an_excluded_mention_cue_still_targets_another_lexical_event() {
    ...
    let result = detector.analyze("@people idiots.", AnalysisContext::for_language("EN"));
...
fn auto_mode_excludes_known_collisions_and_keeps_other_lexical_matches() {
    ...
    let profanity = detector.analyze(
        "Fuck.",
        AnalysisContext {
            language: None,
            reply_target: ReplyTarget::Unknown,
    ...
    let non_english_neutral = detector.analyze(
        "Police.",
        AnalysisContext {
            language: None,
            reply_target: ReplyTarget::Unknown,
...
fn an_explicit_pack_does_not_apply_another_packs_collision_exclusion() {
    ...
    let result = detector.analyze("Police.", AnalysisContext::for_language("EN"));
...
fn an_unsupported_explicit_language_does_not_enter_auto_collision_mode() {
    let detector = detector(vec![entry("FR", "police", "cds", false)]);
    let result = detector.analyze("Police.", AnalysisContext::for_language("JA"));
...
fn duplicate_excluded_rows_emit_one_zero_point_event() {
    ...
    let result = detector.analyze("Love.", AnalysisContext::for_language("EN"));
...
fn displays_public_policy_values_as_lowercase_snake_case() {
    ...
    assert_eq!(
        toxcheck::PolicyCategory::IdentityAttack.to_string(),
        "identity_attack"
    );
    assert_eq!(
...
fn local_hurtlex_detector() -> &'static Detector {
    ...
    DETECTOR.get_or_init(|| {
        ...
        for language in ["AR", "DE", "EN", "ES", "FR", "IT", "PT", "RU"] {
            ...
            let parsed = parse_hurtlex(file, language).unwrap_or_else(|error| {
                panic!(
                    "cannot parse local HurtLex file {}: {error}",
                    path.display()
                )
            });
            entries.extend(
        ...
        Detector::new(entries).expect("local conservative HurtLex detector")
    })
}
