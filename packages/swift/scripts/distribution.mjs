...
const { values: options } = parseArgs({
  options: {
    ...
    packs: { type: "string", default: resolve(projectRoot, "packages/packs/dist") },
    output: { type: "string" },
    repo: { type: "string" },
  },
});
...
function artifacts(directory) {
  const files = readdirSync(directory).filter((name) => extname(name).slice(1) in KINDS).sort();
  const entries = files.map((name) => ({ name, kind: extname(name).slice(1), code: name.slice(0, name.lastIndexOf(".")) }));
...
// Rendered by packages/swift/scripts/distribution.mjs in sospedra/blasphem for ${version}. Do not edit.
...
function render(tree, entries) {
  ...
  for (const { name, kind, code } of entries) {
...
if (entries.length === 0) throw new Error(`${options.packs} holds no .pack or .detect file; run packages/packs/scripts/build.mjs first`);
if (options.output) {
