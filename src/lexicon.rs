...
pub enum ParseLexiconError {
    #[error("cannot parse HurtLex TSV: {0}")]
    Csv(#[from] csv::Error),
    #[error("invalid HurtLex level: {0}")]
    InvalidLevel(String),
    #[error("invalid HurtLex stereotype value: {0}")]
    InvalidStereotype(String),
    #[error("language code is empty")]
    EmptyLanguage,
    #[error("HurtLex row {id} has language {actual}; expected {expected}")]
    LanguageMismatch {
        id: String,
        expected: String,
        actual: String,
