...
use thiserror::Error;
use toxcheck::{Language, LanguageDetector, LanguageIdentifier, LanguageResolution};
...
const ELDC_ARTIFACT_BYTES: u64 = 18_498_380;
const ELDC_ARTIFACT_SHA256: &str =
...
pub struct AutoSizeEvidence {
    ...
    pub eldc_artifact: FileSizeRecord,
...
pub enum AutoEvidenceError {
    ...
    #[error("cannot initialize the ELDC C parity detector: {0}")]
    ParityDetector(eldc::ModelError),
    ...
    #[error("the explicit-only WASM dependency tree contains ELDC")]
    ExplicitTreeContainsEldc,
}
...
/// Reads the paired corpus and records its identity without running ELDC.
///
/// # Errors
///
...
pub fn run_auto_validation(
    ...
    let eldc_artifact = record_file(
        &config.eldc_artifact,
        &relative_label(&config.project_root, &config.eldc_artifact),
        Some(ELDC_ARTIFACT_SHA256),
        Some(ELDC_ARTIFACT_BYTES),
    )?;
...
fn c_parity_evidence(
    ...
    Ok(CParityEvidence {
        ...
        verification_command: "cargo test -p eldc --test parity --locked".to_owned(),
    })
}
...
/// Verifies every frozen C detector field against the Rust ELDC detector.
///
/// # Errors
///
...
pub fn verify_c_parity_fixture(path: &Path) -> Result<u64, AutoEvidenceError> {
    ...
    let detector = eldc::Detector::new().map_err(AutoEvidenceError::ParityDetector)?;
    let mut rows = 0_u64;
...
fn verify_c_parity_row(
    ...
    compare_parity_field(
        ...
        actual.language.map(eldc::Language::code),
    )?;
    compare_parity_field(
        row_number,
        expected,
...
fn explicit_dependency_evidence(
    ...
    let command = "cargo tree -p toxcheck-wasm --no-default-features -e normal";
    ...
    let eldc_present = String::from_utf8_lossy(&output.stdout).contains("eldc v");
    if eldc_present {
        return Err(AutoEvidenceError::ExplicitTreeContainsEldc);
    }
