...
fn size_command_writes_canonical_experimental_evidence() {
    ...
    let status = Command::new(env!("CARGO_BIN_EXE_blasphem-bench"))
        .arg("size")
        .arg("--binary")
        .arg(&binary)
        .arg("--model-manifest")
        .arg(project_root.join("resources/metadata/model-manifest.json"))
        .arg("--lexicon-root")
        .arg(project_root.join("resources/lexicon"))
        .arg("--target-triple")
...
fn auto_command_exposes_the_reproducible_evidence_inputs() {
    let output = Command::new(env!("CARGO_BIN_EXE_blasphem-bench"))
        .arg("auto")
        .arg("--help")
        .output()
