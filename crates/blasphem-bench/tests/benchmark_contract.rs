...
use toxbench::{FixtureLength, assert_unique_dimensions, load_benchmark_fixtures, sha256_hex};
...
fn rate_formulas_use_the_measured_sample_total() {
    let rates = toxbench::calculate_rates(100, 280, 1_000_000_000).expect("rates");
...
fn nearest_rank_percentiles_use_literal_sample_ranks() {
    ...
    assert_eq!(toxbench::nearest_rank(&sorted, 50), Some(5));
    assert_eq!(toxbench::nearest_rank(&sorted, 95), Some(10));
    assert_eq!(toxbench::nearest_rank(&[], 95), None);
}

#[test]
fn fixture_latency_gates_use_strict_length_specific_limits() {
    assert!(toxbench::latency_gate(
...
fn macos_peak_rss_reports_concrete_bytes() {
    assert!(toxbench::peak_rss_bytes().expect("peak RSS") > 0);
}
