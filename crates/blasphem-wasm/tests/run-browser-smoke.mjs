...
const wasmPath = resolve(fullOutput, "toxcheck_wasm_bg.wasm");
const gluePath = resolve(fullOutput, "toxcheck_wasm.js");
const explicitWasmPath = resolve(explicitOutput, "toxcheck_wasm_bg.wasm");
const explicitGluePath = resolve(explicitOutput, "toxcheck_wasm.js");
...
try {
  ...
  await page.goto(
    `http://127.0.0.1:${address.port}/crates/toxcheck-wasm/tests/browser-smoke.html`,
    { waitUntil: "load" },
  );
  await page.waitForFunction(
    () => window.__toxcheckReport?.status !== "running",
    undefined,
    { timeout: 120_000 },
  );
  report = await page.evaluate(() => window.__toxcheckReport);
