use blasphem::{
    LEXICON_SCORE, Language, ReplyTarget, RuleChannel, RuleId, canonical_rule_identity,
};
use sha2::{Digest, Sha256};
use std::path::Path;

fn lexicon(language: Language, rows: &[(&str, &str)]) -> Vec<u8> {
...
fn every_language_resolves_to_one_static_rule_path() {
    for language in Language::ALL {
        RuleChannel::from_lexicon_bytes(language, None)
...
fn every_locked_lexicon_file_builds_its_matching_language_channel() {
    ...
    for language in Language::ALL {
        ...
        RuleChannel::from_lexicon_bytes(language, Some(&bytes))
            .unwrap_or_else(|error| panic!("{}: {error}", language.code()));
    }
}
...
fn supplied_cjk_messages_pass_through_the_shared_channel() {
    ...
    for (language, toxic, clean) in cases {
        ...
        let bytes =
            std::fs::read(&path).unwrap_or_else(|error| panic!("{}: {error}", path.display()));
        let channel =
            RuleChannel::from_lexicon_bytes(language, Some(&bytes)).expect("rule channel");
...
#[test]
fn inclusive_lexicon_rows_cannot_create_rule_events() {
    let bytes = lexicon(Language::En, &[("buffoon", "inclusive")]);
    let channel =
        RuleChannel::from_lexicon_bytes(Language::En, Some(&bytes)).expect("rule channel");
...
#[test]
fn a_lexicon_only_match_scores_below_the_nudge_threshold() {
    let bytes = lexicon(Language::En, &[("buffoon", "conservative")]);
    let channel =
        RuleChannel::from_lexicon_bytes(Language::En, Some(&bytes)).expect("rule channel");

    let result = channel.analyze("buffoon", ReplyTarget::Unknown);

    assert_eq!(result.score, LEXICON_SCORE);
    assert!(!result.should_nudge);
    assert!(
        result
            .evidence
            .iter()
            .any(|item| item.rule_id == RuleId::LexicalMatch && item.points == LEXICON_SCORE)
    );
}

#[test]
fn semantic_and_lexicon_scores_compose_with_maximum() {
    let bytes = lexicon(Language::En, &[("kill", "conservative")]);
    let channel =
        RuleChannel::from_lexicon_bytes(Language::En, Some(&bytes)).expect("rule channel");
...
fn reply_target_completes_a_bare_strong_insult() {
    let channel = RuleChannel::from_lexicon_bytes(Language::En, None).expect("rule channel");
...
#[test]
fn a_wrong_language_lexicon_file_is_rejected() {
    let bytes = lexicon(Language::Ja, &[("問題", "conservative")]);

    assert!(RuleChannel::from_lexicon_bytes(Language::Ko, Some(&bytes)).is_err());
}
...
fn korean_collisions_are_zero_point_evidence() {
    ...
    let channel =
        RuleChannel::from_lexicon_bytes(Language::Ko, Some(&bytes)).expect("rule channel");
    ...
    assert_eq!(report.score, LEXICON_SCORE);
...
fn every_reviewed_cjk_collision_is_excluded() {
    ...
    for (language, lemmas) in cases {
        let rows = lemmas
            ...
            .collect::<Vec<_>>();
        let bytes = lexicon(language, &rows);
        let channel =
            RuleChannel::from_lexicon_bytes(language, Some(&bytes)).expect("rule channel");
        for lemma in lemmas {
...
fn the_audited_german_phrase_reactivates_only_its_lexical_score() {
    let bytes = lexicon(Language::De, &[("hund", "conservative")]);
    let channel =
        RuleChannel::from_lexicon_bytes(Language::De, Some(&bytes)).expect("rule channel");
    ...
    assert_eq!(directed.score, LEXICON_SCORE);
    ...
    assert!(!directed.evidence.iter().any(|item| {
        item.rule_id == RuleId::SemanticDirectedHostility || item.points > LEXICON_SCORE
    }));
}
