...
use blasphem::{Detector, LexiconEntry, MatchLevel, parse_lexicon};
...
#[test]
fn parses_lexicon_rows_with_metadata() {
    let input = concat!(
        "id\tpos\tcategory\tstereotype\tlemma\tlevel\n",
        "ES1\tn\tcds\tyes\tidiota\tconservative\n",
    );

    let entries = parse_lexicon(Cursor::new(input), "es").expect("valid Lexicon data");
