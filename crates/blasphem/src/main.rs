...
enum Command {
    ...
    Judge(JudgeArgs),
    /// Diagnostic policy output over the repository's Lexicon files.
    #[command(hide = true)]
    Check(CheckArgs),
}
...
struct JudgeArgs {
    ...
    text: Option<String>,
    /// Locales to load, comma separated, including id and ms. Default: all 16 languages.
...
fn check(arguments: &CheckArgs) -> Result<()> {
    ...
    let lexicon = fs::read(&path).with_context(|| {
        format!(
            "cannot read required {} Lexicon data at {}",
            language.code(),
            path.display(),
        )
    })?;
