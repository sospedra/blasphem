...
use crate::{
    ...
    PolicyCategory, ReplyTarget, RuleEvidence, RuleId, TextDocument, normalize_text, parse_lexicon,
};

use super::{
    LEXICON_SCORE, LanguageRules, RULE_NUDGE_THRESHOLD, RuleOutcome, analyze_with_rules,
...
/// One immutable semantic and conservative Lexicon rule channel.
...
pub enum RuleChannelError {
    #[error("cannot parse {language} Lexicon data: {source}")]
    ParseLexicon {
        language: &'static str,
        #[source]
        source: ParseLexiconError,
    },
    #[error("cannot build the {language} Lexicon detector: {source}")]
    Detector {
        language: &'static str,
        #[source]
...
impl RuleChannel {
    /// Builds one channel with conservative Lexicon entries only.
    ///
    /// # Errors
    ///
    /// Returns an error for a wrong-language Lexicon file or an invalid matcher.
    pub fn from_lexicon_bytes(
        language: Language,
        lexicon: Option<&[u8]>,
    ) -> Result<Self, RuleChannelError> {
        let rules = resolve_rules(language)?;
        Self::from_resolved_rules(language, lexicon, rules)
    }
    ...
    fn from_resolved_rules(
        ...
        let lexical = lexicon
            .map(|bytes| {
                parse_lexicon(bytes, language.storage_code()).map_err(|source| {
                    RuleChannelError::ParseLexicon {
    ...
    fn analyze_spanish(
        ...
        let rules_only = Detector::rules_only();
        let detector = self.lexical.as_ref().unwrap_or(&rules_only);
        let result = crate::policy::analyze_rule_pack(
            detector,
            text,
            crate::policy::ResolvedRuleContext {
                context: crate::RuleContext {
                    language: Some(pack.language),
                    reply_target,
                },
                pack: Some(pack),
            },
...
fn append_lexical_evidence(
    ...
    for found in &lexical.matches {
        ...
        let points = if excluded { 0 } else { LEXICON_SCORE };
        if !excluded {
            score = LEXICON_SCORE;
        }
