...
fn observe_reads_the_catalog_and_refuses_overwrite() {
    ...
    let first = Command::new(env!("CARGO_BIN_EXE_toxtrain"))
        .args([
            "observe",
            "--source-catalog",
    ...
    let second = Command::new(env!("CARGO_BIN_EXE_toxtrain"))
        .args([
            "observe",
            "--source-catalog",
            catalog.to_str().expect("UTF-8 path"),
...
fn freeze_sources_requires_review_and_refuses_overwrite() {
    ...
    let rejected = Command::new(env!("CARGO_BIN_EXE_toxtrain"))
        .args([
            "freeze-sources",
            "--observation",
            observation
...
fn prepare_textdetox_publishes_all_files_after_deduplication() {
    ...
    let rows = toxtrain::parse_textdetox_rows(File::open(&input_path).expect("open source"))
        .expect("parse source");
    let prepared =
        toxtrain::prepare_textdetox(&rows, &std::collections::BTreeSet::from(["EN".to_owned()]))
            .expect("prepare source");
    toxtrain::publish_prepared_textdetox(&output_directory, &prepared).expect("publish source");
    for name in [
        "development.tsv",
        "validation.tsv",
        "test.tsv",
...
fn prepare_textdetox_refuses_an_existing_output_directory() {
    ...
    let prepared = toxtrain::PreparedTextDetox {
        development: Vec::new(),
        validation: Vec::new(),
        test: Vec::new(),
        provenance: Vec::new(),
        summary: toxtrain::TextDetoxSummary::default(),
    };
    assert!(toxtrain::publish_prepared_textdetox(&output_directory, &prepared).is_err());
...
fn help_exposes_the_task_four_command_contract() {
    let help = Command::new(env!("CARGO_BIN_EXE_toxtrain"))
        .arg("--help")
        .output()
        .expect("run help");
    ...
    let compile = Command::new(env!("CARGO_BIN_EXE_toxtrain"))
        .args(["compile", "--help"])
        .output()
    ...
    let eval = Command::new(env!("CARGO_BIN_EXE_toxtrain"))
        .args(["eval", "--help"])
        .output()
...
fn toxtrain_command(arguments: &[&str]) -> std::process::Output {
    Command::new(env!("CARGO_BIN_EXE_toxtrain"))
...
fn write_catalog_observation(path: &std::path::Path) {
    ...
    let catalog: toxtrain::source_manifest::SourceCatalog =
        serde_json::from_reader(File::open(catalog_path).expect("open catalog"))
            .expect("parse catalog");
    let digest = toxtrain::evidence::Sha256Digest::try_from(
        "0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef".to_owned(),
    )
    .expect("digest");
    let observation = toxtrain::source_manifest::SourceObservation {
        schema_version: "source-observation-v1".to_owned(),
        sources: catalog
            .sources
            .into_iter()
            .map(|source| toxtrain::source_manifest::SourceRecord {
                ...
                download_sha256: (source.dataset == toxtrain::datasets::DatasetId::TextDetox)
                    .then(|| digest.clone()),
...
fn write_prepare_fixture(
    ...
    use toxcheck::{Language, normalize_text};
    use toxtrain::{
    ...
    let mut add_source = |dataset, language, id: &str, path: &str| {
        records.push(SourceRecord {
            ...
            revision: if dataset == DatasetId::TextDetox {
                Some(toxtrain::TEXTDETOX_REVISION.to_owned())
            } else {
    ...
    for (code, language) in textdetox {
        ...
        for (index, row) in rows.into_iter().enumerate() {
            let source_id = format!(
                ...
                toxtrain::TEXTDETOX_REVISION
            );
            if code == "en" && audit_source_id.is_none() {
                let normalized = normalize_text(&row.text);
                if toxtrain::datasets::split_for_key(language, &normalized)
                    == toxtrain::datasets::DatasetSplit::Development
                {
...
fn hash_fixture_rows(
    ...
    for toxic in [false, true] {
        ...
        for index in 0..100_000 {
            ...
            let normalized = toxcheck::normalize_text(&text);
            let remainder = toxtrain::datasets::split_hash(language, &normalized) % 100;
    ...
    if !turkish {
        for toxic in [false, true] {
            ...
            for index in 100_000..200_000 {
                ...
                let normalized = toxcheck::normalize_text(&text);
                if toxtrain::datasets::split_for_key(language, &normalized)
                    == toxtrain::datasets::DatasetSplit::Test
                {
                    rows.push(FixtureRow { toxic, text });
                    test += 1;
                }
...
fn turkish_training_rows() -> Vec<FixtureRow> {
    hash_fixture_rows(toxcheck::Language::Tr, "turkish train", true, 1)
}

fn write_preserved_fixtures(
    raw_root: &std::path::Path,
    add_source: &mut impl FnMut(toxtrain::datasets::DatasetId, toxcheck::Language, &str, &str),
    (dataset, language, prefix, paths): (
        toxtrain::datasets::DatasetId,
        toxcheck::Language,
        &str,
        [&str; 3],
    ),
) {
    let source_ids = if dataset == toxtrain::datasets::DatasetId::ViHos {
    ...
    for (path, split, count) in [
        ...
        let mut content = if dataset == toxtrain::datasets::DatasetId::ViHos {
            ",content,index_spans\n".to_owned()
        } else {
            "document\tlabel\n".to_owned()
        };
        for index in 0..count {
            ...
            if dataset == toxtrain::datasets::DatasetId::ViHos {
                writeln!(
                    content,
                    "{split}-{index},{text},{}",
                    if toxic { "[0]" } else { "[]" }
