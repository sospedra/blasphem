...
fn publication_excludes_audit_only_rows_from_split_counts() {
    ...
    audit_only.exclusion_reason = Some(toxtrain::datasets::ExclusionReason::AuditOnly);
...
fn publication_rejects_duplicate_prepared_source_ids_in_one_split() {
    ...
    assert!(matches!(
        error,
        toxtrain::PreparedPublicationError::DuplicatePreparedSourceId(source_id)
...
fn publication_rejects_a_prepared_row_for_another_language() {
    ...
    assert!(matches!(
        error,
        toxtrain::PreparedPublicationError::PreparedRowLanguageMismatch {
            source_id,
            actual: Language::Es,
...
fn publication_rejects_blank_required_provenance_strings() {
    for field in [
        ...
        assert!(matches!(
            error,
            toxtrain::PreparedPublicationError::BlankRequiredProvenanceField(actual)
                if actual == field
        ));
        assert!(!output.exists());
    }
...
fn publish_fixture(output: &Path) -> toxtrain::publication::PreparedPublicationResult {
