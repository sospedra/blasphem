...
fn set_error(message: String) {
    let text = CString::new(message.replace('\0', " ")).expect("NUL bytes replaced");
...
pub unsafe extern "C" fn blasphem_builder_add(
    ...
    let (Some(locale), Some(pack)) = (locale, pack) else {
        set_error("BLASPHEM_PACK_INVALID: locale and pack bytes are required".to_owned());
        return 1;
    };
...
/// Consumes the builder. Returns null on failure with the message in
/// `blasphem_last_error`.
///
/// # Safety
///
/// `builder` must come from `blasphem_builder_new` and is invalid afterwards.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn blasphem_builder_build(
    ...
    // SAFETY: the caller hands over ownership of a live builder.
    let builder = unsafe { Box::from_raw(builder) };
    match Engine::build(&builder.sources, builder.detect_language, builder.grawlix) {
        Ok(engine) => Box::into_raw(Box::new(blasphem_engine { engine })),
        Err(error) => {
            set_error(error.to_string());
            ptr::null_mut()
        }
...
/// Frees a builder that was never built.
///
/// # Safety
///
/// `builder` must come from `blasphem_builder_new`, unbuilt, and is invalid afterwards.
#[unsafe(no_mangle)]
