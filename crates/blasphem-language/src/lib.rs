...
static EMBEDDED_MODEL: &[u8] = include_bytes!("../data/eldc-15-v1.bin");

/// A language profile in the compact ELDC model.
...
impl Language {
    ...
    /// Returns the lowercase ISO 639-1 code used by ELDC.
    #[must_use]
...
impl Display for ModelError {
    fn fmt(&self, formatter: &mut Formatter<'_>) -> fmt::Result {
        match self {
            Self::InvalidMagic => formatter.write_str("the ELDC model magic is invalid"),
            Self::UnsupportedVersion(version) => {
                write!(formatter, "the ELDC model version {version} is unsupported")
            }
            Self::InvalidLanguageCount(count) => {
                write!(
                    formatter,
                    "the ELDC model language count {count} is invalid"
                )
            }
            Self::InvalidTableLength(length) => {
                write!(formatter, "the ELDC model table length {length} is invalid")
            }
            Self::InvalidLetterTableLength(length) => {
                write!(
                    formatter,
                    "the ELDC letter table length {length} is invalid"
                )
            }
            Self::InvalidCjkTableLength(length) => {
                write!(formatter, "the ELDC CJK table length {length} is invalid")
            }
            Self::InvalidLowercaseTableLength(length) => {
                write!(
                    formatter,
                    "the ELDC lowercase table length {length} is invalid"
                )
            }
            Self::InvalidSourceCommit => formatter.write_str("the ELDC source commit is invalid"),
            Self::InvalidEmptySlot { slot } => {
                write!(formatter, "the ELDC empty slot {slot} has metadata")
            }
            Self::InvalidBlobRange { slot } => {
                write!(formatter, "the ELDC slot {slot} has an invalid blob range")
            }
            Self::InvalidLanguageIndex { index } => {
                write!(
                    formatter,
                    "the ELDC blob item {index} has an invalid language"
                )
            }
            Self::Truncated => formatter.write_str("the ELDC model is truncated"),
            Self::TrailingData => formatter.write_str("the ELDC model has trailing data"),
        }
    }
}
